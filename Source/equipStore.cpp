...
void equipStore::BuyEquip()
{
	if (_click == CLICKE::SELECT && money >= _vEquip[0]->money)
	{
		...
		_viBagEquip = _vBagEquip.begin();
		for (_viBagEquip; _viBagEquip != _vBagEquip.end(); ++_viBagEquip)
		{
			if ((*_viBagEquip)->name == _vEquip[0]->name)
			{
				isExist = true;
				(*_viBagEquip)->num = _equipWindow->buyEquip_num;

				if ((*_viBagEquip)->num == 0)
				{
					_click = CLICKE::NONE;
					_vEquip.erase(_viBagEquip);
					SAFE_DELETE(*_viBagEquip);
				}
			}
		}
	...
	if (_click == CLICKE::SELECT1 && money >= _vEquip[1]->money)
	{
		...
		for (_viBagEquip; _viBagEquip != _vBagEquip.end(); ++_viBagEquip)
		{
			if ((*_viBagEquip)->name == _vEquip[1]->name)
			{
				isExist = true;
				(*_viBagEquip)->num = _equipWindow->buyEquip_num;
	...
	if (_click == CLICKE::SELECT2 && money >= _vEquip[2]->money)
	{
		...
		for (_viBagEquip; _viBagEquip != _vBagEquip.end(); ++_viBagEquip)
		{
			if ((*_viBagEquip)->name == _vEquip[2]->name)
			{
				isExist = true;
				(*_viBagEquip)->num = _equipWindow->buyEquip_num;
	...
	if (_click == CLICKE::SELECT3 && money >= _vEquip[3]->money)
	{
		...
		for (_viBagEquip; _viBagEquip != _vBagEquip.end(); ++_viBagEquip)
		{
			if ((*_viBagEquip)->name == _vEquip[3]->name)
			{
				isExist = true;
				(*_viBagEquip)->num = _equipWindow->buyEquip_num;
	...
	if (_click == CLICKE::SELECT4 && money >= _vEquip[4]->money)
	{
		...
		for (_viBagEquip; _viBagEquip != _vBagEquip.end(); ++_viBagEquip)
		{
			if ((*_viBagEquip)->name == _vEquip[4]->name)
			{
				isExist = true;
				(*_viBagEquip)->num = _equipWindow->buyEquip_num;
	...
	if (_click == CLICKE::SELECT5 && money >= _vEquip[5]->money)
	{
		...
		for (_viBagEquip; _viBagEquip != _vBagEquip.end(); ++_viBagEquip)
		{
			if ((*_viBagEquip)->name == _vEquip[5]->name)
			{
				isExist = true;
				(*_viBagEquip)->num = _equipWindow->buyEquip_num;
	...
	if (_click == CLICKE::SELECT6 && money >= _vEquip[6]->money)
	{
		...
		for (_viBagEquip; _viBagEquip != _vBagEquip.end(); ++_viBagEquip)
		{
			if ((*_viBagEquip)->name == _vEquip[6]->name)
			{
				isExist = true;
				(*_viBagEquip)->num = _equipWindow->buyEquip_num;
	...
	if (_click == CLICKE::SELECT7 && money >= _vEquip[7]->money)
	{
		...
		for (_viBagEquip; _viBagEquip != _vBagEquip.end(); ++_viBagEquip)
		{
			if ((*_viBagEquip)->name == _vEquip[7]->name)
			{
				isExist = true;
				(*_viBagEquip)->num = _equipWindow->buyEquip_num;
