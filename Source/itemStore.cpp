	...
	if (activeTime >= 80 && !(unitList_active))
	{
		if (InputManager->isOnceKeyUp(VK_LBUTTON))
		{
			...
			if (_itemWindow->isBuyButton)
			{
				...
				{
					if (_click == CLICK::SELECT && money >= (_vStoreItem[0]->money * _itemWindow->buyItem_num) + _vStoreItem[0]->money) { _itemWindow->buyItem_num++; }
					if (_click == CLICK::SELECT1 && money >= (_vStoreItem[1]->money * _itemWindow->buyItem_num) + _vStoreItem[1]->money) { _itemWindow->buyItem_num++; }
					if (_click == CLICK::SELECT2 && money >= (_vStoreItem[2]->money * _itemWindow->buyItem_num) + _vStoreItem[2]->money) { _itemWindow->buyItem_num++; }
					if (_click == CLICK::SELECT3 && money >= (_vStoreItem[3]->money * _itemWindow->buyItem_num) + _vStoreItem[3]->money) { _itemWindow->buyItem_num++; }
					if (_click == CLICK::SELECT4 && money >= (_vStoreItem[4]->money * _itemWindow->buyItem_num) + _vStoreItem[4]->money) { _itemWindow->buyItem_num++; }
					if (_click == CLICK::SELECT5 && money >= (_vStoreItem[5]->money * _itemWindow->buyItem_num) + _vStoreItem[5]->money) { _itemWindow->buyItem_num++; }
					if (_click == CLICK::SELECT6 && money >= (_vStoreItem[6]->money * _itemWindow->buyItem_num) + _vStoreItem[6]->money) { _itemWindow->buyItem_num++; }
					if (_click == CLICK::SELECT7 && money >= (_vStoreItem[7]->money * _itemWindow->buyItem_num) + _vStoreItem[7]->money) { _itemWindow->buyItem_num++; }
				}
...
void itemStore::render()
{
	...
	if (isOpen && !(_itemWindow->isClosed))
	{
		...
		{
			TextOut(getMemDC(), _itemWindow->itemSelect[i].left + 50, _itemWindow->itemSelect[i].top + 10, _vStoreItem[i]->name.c_str(), lstrlen(_vStoreItem[i]->name.c_str()));
		}

		//가격 텍스트
		for (int i = 0; i < ITEMMAX; i++)
		{
			TextOut(getMemDC(), _itemWindow->itemSelect[i].left + 300, _itemWindow->itemSelect[i].top + 10, to_string(_vStoreItem[i]->money).c_str(), lstrlen(to_string(_vStoreItem[i]->money).c_str()));
		}
		...
		if (_click == CLICK::SELECT) 	{ DrawText(getMemDC(), _vStoreItem[0]->detail.c_str(), -1, &temp_des, DT_LEFT | DT_WORDBREAK | DT_EDITCONTROL); _click == CLICK::NONE; }
		if (_click == CLICK::SELECT1)	{ DrawText(getMemDC(), _vStoreItem[1]->detail.c_str(), -1, &temp_des, DT_LEFT | DT_WORDBREAK | DT_EDITCONTROL); _click == CLICK::NONE; }
		if (_click == CLICK::SELECT2)	{ DrawText(getMemDC(), _vStoreItem[2]->detail.c_str(), -1, &temp_des, DT_LEFT | DT_WORDBREAK | DT_EDITCONTROL); _click == CLICK::NONE; }
		if (_click == CLICK::SELECT3)	{ DrawText(getMemDC(), _vStoreItem[3]->detail.c_str(), -1, &temp_des, DT_LEFT | DT_WORDBREAK | DT_EDITCONTROL); _click == CLICK::NONE; }
		if (_click == CLICK::SELECT4)	{ DrawText(getMemDC(), _vStoreItem[4]->detail.c_str(), -1, &temp_des, DT_LEFT | DT_WORDBREAK | DT_EDITCONTROL); _click == CLICK::NONE; }
		if (_click == CLICK::SELECT5)	{ DrawText(getMemDC(), _vStoreItem[5]->detail.c_str(), -1, &temp_des, DT_LEFT | DT_WORDBREAK | DT_EDITCONTROL); _click == CLICK::NONE; }
		if (_click == CLICK::SELECT6)	{ DrawText(getMemDC(), _vStoreItem[6]->detail.c_str(), -1, &temp_des, DT_LEFT | DT_WORDBREAK | DT_EDITCONTROL); _click == CLICK::NONE; }
		if (_click == CLICK::SELECT7)	{ DrawText(getMemDC(), _vStoreItem[7]->detail.c_str(), -1, &temp_des, DT_LEFT | DT_WORDBREAK | DT_EDITCONTROL); _click == CLICK::NONE; }
	...
	if (_itemWindow->isBuyButton)
	{
		...
		*/
		...
		{
			_viStoreItem = _vStoreItem.begin();
			wsprintf(str, "%d", ((_vStoreItem[0])->money) * _itemWindow->buyItem_num);
		...
		{
			_viStoreItem = _vStoreItem.begin() + 1;
			wsprintf(str, "%d", ((_vStoreItem[1])->money)* _itemWindow->buyItem_num);
			TextOut(getMemDC(), _itemWindow->buyWin_name.left + 120, _itemWindow->buyWin_name.bottom + 84, str, lstrlen(str));
		}
		else if (_click == CLICK::SELECT2)
		{
			_viStoreItem = _vStoreItem.begin() + 2;
			wsprintf(str, "%d", ((_vStoreItem[2])->money) * _itemWindow->buyItem_num);
			TextOut(getMemDC(), _itemWindow->buyWin_name.left + 120, _itemWindow->buyWin_name.bottom + 84, str, lstrlen(str));
		}
		else if (_click == CLICK::SELECT3)
		{
			_viStoreItem = _vStoreItem.begin() + 3;
			wsprintf(str, "%d", ((_vStoreItem[3])->money) * _itemWindow->buyItem_num);
			TextOut(getMemDC(), _itemWindow->buyWin_name.left + 120, _itemWindow->buyWin_name.bottom + 84, str, lstrlen(str));
		}
		else if (_click == CLICK::SELECT4)
		{
			_viStoreItem = _vStoreItem.begin() + 4;
			wsprintf(str, "%d", ((_vStoreItem[4])->money) * _itemWindow->buyItem_num);
			TextOut(getMemDC(), _itemWindow->buyWin_name.left + 120, _itemWindow->buyWin_name.bottom + 84, str, lstrlen(str));
		}
		else if (_click == CLICK::SELECT5)
		{
			_viStoreItem = _vStoreItem.begin() + 5;
			wsprintf(str, "%d", ((_vStoreItem[5])->money) * _itemWindow->buyItem_num);
			TextOut(getMemDC(), _itemWindow->buyWin_name.left + 120, _itemWindow->buyWin_name.bottom + 84, str, lstrlen(str));
		}
		else if (_click == CLICK::SELECT6)
		{
			_viStoreItem = _vStoreItem.begin() + 6;
			wsprintf(str, "%d", ((_vStoreItem[6])->money) * _itemWindow->buyItem_num);
			TextOut(getMemDC(), _itemWindow->buyWin_name.left + 120, _itemWindow->buyWin_name.bottom + 84, str, lstrlen(str));
		}
		else if (_click == CLICK::SELECT7)
		{
			_viStoreItem = _vStoreItem.begin() + 7;
			wsprintf(str, "%d", ((_vStoreItem[7])->money) * _itemWindow->buyItem_num);
			TextOut(getMemDC(), _itemWindow->buyWin_name.left + 120, _itemWindow->buyWin_name.bottom + 84, str, lstrlen(str));
		}
...
{
	if (_click == CLICK::SELECT && money >= _vStoreItem[0]->money)
	{
		//이미 존재하는지 판별.
		bool isExist = false;

		_viBagItem = _vBagItem.begin();
		for (_viBagItem; _viBagItem != _vBagItem.end(); ++_viBagItem)
		{
			if ((*_viBagItem)->name == _vStoreItem[0]->name)
			{
				isExist = true;
				(*_viBagItem)->num += _itemWindow->buyItem_num;

				if ((*_viBagItem)->num == 0)
				{
					_click = CLICK::NONE;
					_vBagItem.erase(_viBagItem);
					SAFE_DELETE(*_viBagItem);
				}
			}
		}
		...
		if (!(isExist))
		{
			tagItemInfo* item;
			item = new tagItemInfo;
			item->name = _vStoreItem[0]->name;
			item->detail = _vStoreItem[0]->detail;
			item->num = _itemWindow->buyItem_num;
			item->money = _vStoreItem[0]->money;
			item->iconImg = _vStoreItem[0]->iconImg;

			_vBagItem.push_back(item);
		}
		_viStoreItem = _vStoreItem.begin();
		money -= ((_vStoreItem[0]->money) * _itemWindow->buyItem_num);
		(_vStoreItem[0])->num += _itemWindow->buyItem_num;
	}

	if (_click == CLICK::SELECT1 && money >= _vStoreItem[1]->money)
	{
		//이미 존재하는지 판별.
		bool isExist = false;
		...
		for (_viBagItem; _viBagItem != _vBagItem.end(); ++_viBagItem)
		{
			if ((*_viBagItem)->name == _vStoreItem[1]->name)
			{
				isExist = true;
				(*_viBagItem)->num += _itemWindow->buyItem_num;
		...
		if (!(isExist))
		{
			tagItemInfo* item;
			item = new tagItemInfo;
			item->name = _vStoreItem[1]->name;
			item->detail = _vStoreItem[1]->detail;
			item->num = _itemWindow->buyItem_num;
			item->money = _vStoreItem[1]->money;
			item->iconImg = _vStoreItem[1]->iconImg;

			_vBagItem.push_back(item);
		}

		_viStoreItem = _vStoreItem.begin();
		money -= ((_vStoreItem[1]->money) * _itemWindow->buyItem_num);
		//_unitList->setMoney(_unitList->getMoney() - ((_vItem[1]->money) * _itemWindow->buyItem_num));

		(_vStoreItem[1])->num += _itemWindow->buyItem_num;

		if ((_vStoreItem[1])->num == 0)
		{
			_click = CLICK::NONE;
			_vStoreItem.erase(_viStoreItem);
		}
	}
	else if (_click == CLICK::SELECT2 && money >= _vStoreItem[2]->money)
	{
		//이미 존재하는지 판별.
		bool isExist = false;
		...
		for (_viBagItem; _viBagItem != _vBagItem.end(); ++_viBagItem)
		{
			if ((*_viBagItem)->name == _vStoreItem[2]->name)
			{
				isExist = true;
				(*_viBagItem)->num += _itemWindow->buyItem_num;
		...
		if (!(isExist))
		{
			tagItemInfo* item;
			item = new tagItemInfo;
			item->name = _vStoreItem[2]->name;
			item->detail = _vStoreItem[2]->detail;
			item->num = _itemWindow->buyItem_num;
			item->money = _vStoreItem[2]->money;
			item->iconImg = _vStoreItem[2]->iconImg;

			_vBagItem.push_back(item);
		}

		_viStoreItem = _vStoreItem.begin();
		money -= ((_vStoreItem[2]->money) * _itemWindow->buyItem_num);
		//_unitList->setMoney(_unitList->getMoney() - ((_vItem[2]->money) * _itemWindow->buyItem_num));

		(_vStoreItem[2])->num += _itemWindow->buyItem_num;

		if ((_vStoreItem[2])->num == 0)
		{
			_click = CLICK::NONE;
			_vStoreItem.erase(_viStoreItem);
		}
	}
	else if (_click == CLICK::SELECT3 && money >= _vStoreItem[3]->money)
	{
		//이미 존재하는지 판별.
		bool isExist = false;
		...
		for (_viBagItem; _viBagItem != _vBagItem.end(); ++_viBagItem)
		{
			if ((*_viBagItem)->name == _vStoreItem[3]->name)
			{
				isExist = true;
				(*_viBagItem)->num += _itemWindow->buyItem_num;
		...
		if (!(isExist))
		{
			tagItemInfo* item;
			item = new tagItemInfo;
			item->name = _vStoreItem[3]->name;
			item->detail = _vStoreItem[3]->detail;
			item->num = _itemWindow->buyItem_num;
			item->money = _vStoreItem[3]->money;
			item->iconImg = _vStoreItem[3]->iconImg;

			_vBagItem.push_back(item);
		}

		_viStoreItem = _vStoreItem.begin();
		money -= ((_vStoreItem[3]->money) * _itemWindow->buyItem_num);
		//_unitList->setMoney(_unitList->getMoney() - ((_vItem[3]->money) * _itemWindow->buyItem_num));

		(_vStoreItem[3])->num += _itemWindow->buyItem_num;

		if ((_vStoreItem[3])->num == 0)
		{
			_click = CLICK::NONE;
			_vStoreItem.erase(_viStoreItem);
		}
	}
	else if (_click == CLICK::SELECT4 && money >= _vStoreItem[4]->money)
	{
		//이미 존재하는지 판별.
		bool isExist = false;
		...
		for (_viBagItem; _viBagItem != _vBagItem.end(); ++_viBagItem)
		{
			if ((*_viBagItem)->name == _vStoreItem[4]->name)
			{
				isExist = true;
				(*_viBagItem)->num += _itemWindow->buyItem_num;
		...
		if (!(isExist))
		{
			tagItemInfo* item;
			item = new tagItemInfo;
			item->name = _vStoreItem[4]->name;
			item->detail = _vStoreItem[4]->detail;
			item->num = _itemWindow->buyItem_num;
			item->money = _vStoreItem[4]->money;
			item->iconImg = _vStoreItem[4]->iconImg;

			_vBagItem.push_back(item);
		}

		_viStoreItem = _vStoreItem.begin();
		money -= ((_vStoreItem[4]->money) * _itemWindow->buyItem_num);
		//_unitList->setMoney(_unitList->getMoney() - ((_vItem[4]->money) * _itemWindow->buyItem_num));

		(_vStoreItem[4])->num += _itemWindow->buyItem_num;

		if ((_vStoreItem[4])->num == 0)
		{
			_click = CLICK::NONE;
			_vStoreItem.erase(_viStoreItem);
		}
	}
	else if (_click == CLICK::SELECT5 && money >= _vStoreItem[5]->money)
	{
		//이미 존재하는지 판별.
		bool isExist = false;
		...
		for (_viBagItem; _viBagItem != _vBagItem.end(); ++_viBagItem)
		{
			if ((*_viBagItem)->name == _vStoreItem[5]->name)
			{
				isExist = true;
				(*_viBagItem)->num += _itemWindow->buyItem_num;
		...
		if (!(isExist))
		{
			...
			item = new tagItemInfo;
			item->name = "치유약";
			item->detail = _vStoreItem[5]->detail;
			item->num = _itemWindow->buyItem_num;
			item->money = _vStoreItem[5]->money;
			item->iconImg = _vStoreItem[5]->iconImg;

			_vBagItem.push_back(item);
		}

		_viStoreItem = _vStoreItem.begin();
		money -= ((_vStoreItem[5]->money) * _itemWindow->buyItem_num);
		//_unitList->setMoney(_unitList->getMoney() - ((_vItem[5]->money) * _itemWindow->buyItem_num));

		(_vStoreItem[5])->num += _itemWindow->buyItem_num;

		if ((_vStoreItem[5])->num == 0)
		{
			_click = CLICK::NONE;
			_vStoreItem.erase(_viStoreItem);
		}
	}
	else if (_click == CLICK::SELECT6 && money >= _vStoreItem[6]->money)
	{
		//이미 존재하는지 판별.
		bool isExist = false;
		...
		for (_viBagItem; _viBagItem != _vBagItem.end(); ++_viBagItem)
		{
			if ((*_viBagItem)->name == _vStoreItem[6]->name)
			{
				isExist = true;
				(*_viBagItem)->num += _itemWindow->buyItem_num;
		...
		if (!(isExist))
		{
			tagItemInfo* item;
			item = new tagItemInfo;
			item->name = _vStoreItem[6]->name;
			item->detail = _vStoreItem[6]->detail;
			item->num = _itemWindow->buyItem_num;
			item->money = _vStoreItem[6]->money;
			item->iconImg = _vStoreItem[6]->iconImg;

			_vBagItem.push_back(item);
		}

		_viStoreItem = _vStoreItem.begin();
		money -= ((_vStoreItem[6]->money) * _itemWindow->buyItem_num);
		//_unitList->setMoney(_unitList->getMoney() - ((_vItem[6]->money) * _itemWindow->buyItem_num));

		(_vStoreItem[6])->num += _itemWindow->buyItem_num;

		if ((_vStoreItem[6])->num == 0)
		{
			_click = CLICK::NONE;
			_vStoreItem.erase(_viStoreItem);
		}
	}
	else if (_click == CLICK::SELECT7 && money >= _vStoreItem[7]->money)
	{
		//이미 존재하는지 판별.
		bool isExist = false;
		...
		for (_viBagItem; _viBagItem != _vBagItem.end(); ++_viBagItem)
		{
			if ((*_viBagItem)->name == _vStoreItem[7]->name)
			{
				isExist = true;
				(*_viBagItem)->num += _itemWindow->buyItem_num;
		...
		if (!(isExist))
		{
			tagItemInfo* item;
			item = new tagItemInfo;
			item->name = _vStoreItem[7]->name;
			item->detail = _vStoreItem[7]->detail;
			item->num = _itemWindow->buyItem_num;
			item->money = _vStoreItem[7]->money;
			item->iconImg = _vStoreItem[7]->iconImg;

			_vBagItem.push_back(item);
		}

		_viStoreItem = _vStoreItem.begin();
		money -= ((_vStoreItem[7]->money) * _itemWindow->buyItem_num);
		//_unitList->setMoney(_unitList->getMoney() - ((_vItem[7]->money) * _itemWindow->buyItem_num));

		(_vStoreItem[7])->num += _itemWindow->buyItem_num;

		if ((_vStoreItem[7])->num == 0)
		{
			_click = CLICK::NONE;
			_vStoreItem.erase(_viStoreItem);
		}
