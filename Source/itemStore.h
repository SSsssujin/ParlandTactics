...
public:
	...
	//아이템 벡터
	vector<tagItemInfo*> _vStoreItem;
	vector<tagItemInfo*>::iterator _viStoreItem;
	vector<tagItemInfo*> getVItem() const { return _vStoreItem; }

	//아이템 산 순서대로 벡터에 넣기
	vector<tagItemInfo*> _vBagItem;
	vector<tagItemInfo*>::iterator _viBagItem;
	vector<tagItemInfo*> getVTools() const { return _vBagItem; }
	void setVTools(vector<tagItemInfo*> vTools) { _vBagItem = vTools; }
